...
#+--- Bars LEFT ---+
# Session name
tmux set -g status-left "#[fg=${THEME[bblack]},bg=${THEME[blue]},nobold] #{?client_prefix,󰠠 ,#[dim]󰤂 }#[nobold,nodim]#S$hostname "

#+--- Windows ---+
# Focused (active) window
tmux set -g window-status-current-format "$RESET#[fg=green,bg=default] #{?#{==:#{pane_current_command},ssh},󰣀 ,$active_terminal_icon $window_space}#[fg=${THEME[foreground]},bold,nodim]$window_number#W#[nobold]#{?window_zoomed_flag, $zoom_number, $custom_pane}#{?window_last_flag, , }"
